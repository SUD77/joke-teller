...
function tellMe(joke) {
    ...
    VoiceRSS.speech({
        key: '02d2495e31d44c3787d9f91614fff026',    //My unique APIkey
        src: joke,
        hl: 'en-us',
        v: 'Linda',
        r: 0, 
...
async function getJokes() {
    ...
    const apiUrl = 'https://v2.jokeapi.dev/joke/Miscellaneous,Dark,Pun,Spooky';     //taking data in apiURl
    try {
        const response = await fetch(apiUrl);               
        const data = await response.json();         //taking data from json into data vriable
        if (data.setup) {
            joke = `${data.setup} ... ${data.delivery}`;   //seup and delivery are part of joke in json
        } else {
            joke = data.joke;
